...
function start() {
  ...
  // Step 1. Initialize the app and Activate the Agent
  // --------------------------------------------
  readConfigurationParams();
  activateTheAgent();
 
  // Step 2. Register the ROS 2 System in the IoTA
  // ----------------------------------------
  provisionROS2Service();
  provisionROS2System();
  waitUntilTheIotaIsReady();
 
  // Step 3. Setup and Run the ROS 2 middleware (South Port of the Agent)
  // --------------------------------------------------------------------
  // Create the ROS 2 Node which will handle the south port of the IoT Agent
...
function startTheBridge(){
  ...
  var subscriber_keys = Object.keys(ros2_system_conf.ubscribers);
